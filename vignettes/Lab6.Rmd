...
The rate of making a type 1 error in this set up where notably no correction for multiple testing is performed is for 50 subjects (/trials) is `r length(D[D > 0])/10000`.
...
```
This looks like an F distribution.
...
Close to the same! `r (length(omnibus_pvalues[omnibus_pvalues < .05])/10000)*100`% of the time you will make type 1 error in this design for omnibus versus `r (length(contrast_pvalues[contrast_pvalues < .05])/10000)*100`% of time for contrast. 
